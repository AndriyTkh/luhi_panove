...
describe('AuthService', () => {
    ...
    describe('findOrCreateGuestUser', () => {
        ...
        test('handles non-empty IP string', async () => {
            const validIp = '192.168.1.109';
            const user = await authService.findOrCreateGuestUser(validIp);
            expect(user).toBeDefined();
            expect(user.ip).toBe(validIp);
        });
    });
