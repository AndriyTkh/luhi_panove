...
import { ServerError, RateLimitError } from '../../../src/middleware/errorHandler';
import { Iteration } from '../../../src/types';
...
describe('GeminiService', () => {
  ...
  describe('parseAIResponse', () => {
    ...
    test('throws ServerError when response is not JSON', () => {
      ...
      expect(() => geminiService.parseAIResponse(response)).toThrow(ServerError);
    });
  ...
  describe('generateRanking', () => {
    ...
    test('generates different values on multiple calls', () => {
      ...
      const isDifferent =
        ranking1.originality !== ranking2.originality ||
