...
const IdeaService_1 = require("../../../src/services/IdeaService");
...
describe('IdeaService', () => {
    ...
    let otherUserId;
    beforeEach(async () => {
        ideaService = new IdeaService_1.IdeaService();
    ...
    describe('createIdea', () => {
        test('creates idea with initial iteration version 1', async () => {
            ...
            expect(idea.iterations[0].ranking.originality).toBe(0);
            expect(idea.iterations[0].ranking.difficulty).toBe(0);
            expect(idea.iterations[0].ranking.marketPotential).toBe(0);
            expect(idea.iterations[0].ranking.scalability).toBe(0);
        });
        test('creates idea with minimal valid title', async () => {
            const idea = await ideaService.createIdea(userId, 'A', 'Description');
            expect(idea.iterations[0].title).toBe('A');
        });
        test('creates idea with minimal valid description', async () => {
            const idea = await ideaService.createIdea(userId, 'Title', 'A');
            expect(idea.iterations[0].description).toBe('A');
        });
    ...
    describe('getIdeaById', () => {
        ...
        test('throws NotFoundError when idea does not exist', async () => {
            ...
            await expect(ideaService.getIdeaById(nonExistentId, userId)).rejects.toThrow(errorHandler_1.NotFoundError);
        });
        test('throws AuthorizationError when idea belongs to different user', async () => {
            const created = await ideaService.createIdea(userId, 'Test', 'Description');
            await expect(ideaService.getIdeaById(created._id.toString(), otherUserId)).rejects.toThrow(errorHandler_1.AuthorizationError);
        });
    });
    describe('deleteIdea', () => {
        test('deletes idea when it belongs to user', async () => {
            ...
            await expect(ideaService.getIdeaById(created._id.toString(), userId)).rejects.toThrow(errorHandler_1.NotFoundError);
        });
        test('throws NotFoundError when idea does not exist', async () => {
            const nonExistentId = '507f1f77bcf86cd799439011';
            await expect(ideaService.deleteIdea(nonExistentId, userId)).rejects.toThrow(errorHandler_1.NotFoundError);
        });
        test('throws AuthorizationError when idea belongs to different user', async () => {
            const created = await ideaService.createIdea(userId, 'Test', 'Description');
            await expect(ideaService.deleteIdea(created._id.toString(), otherUserId)).rejects.toThrow(errorHandler_1.AuthorizationError);
        });
    });
    describe('addIteration', () => {
        ...
        test('throws NotFoundError when idea does not exist', async () => {
            const nonExistentId = '507f1f77bcf86cd799439011';
            await expect(ideaService.addIteration(nonExistentId, {
                title: 'Test',
                description: 'Test',
                plan: [],
        ...
        test('handles boundary ranking values (0 and 100)', async () => {
            ...
            expect(updated.iterations[1].ranking.originality).toBe(0);
            expect(updated.iterations[1].ranking.difficulty).toBe(100);
            expect(updated.iterations[1].ranking.marketPotential).toBe(0);
            expect(updated.iterations[1].ranking.scalability).toBe(100);
        });
    });
    describe('updateIteration', () => {
        test('updates title only', async () => {
            ...
            const updated = await ideaService.updateIteration(created._id.toString(), 1, { title: 'Updated Title' });
        ...
        test('updates description only', async () => {
            ...
            const updated = await ideaService.updateIteration(created._id.toString(), 1, { description: 'Updated Description' });
        ...
        test('updates plan only', async () => {
            ...
            const updated = await ideaService.updateIteration(created._id.toString(), 1, { plan: ['New Step 1', 'New Step 2'] });
            expect(updated.iterations[0].plan).toEqual(['New Step 1', 'New Step 2']);
        });
        test('updates multiple fields at once', async () => {
            const created = await ideaService.createIdea(userId, 'Original', 'Original Description');
            const updated = await ideaService.updateIteration(created._id.toString(), 1, {
                title: 'Updated Title',
        ...
        test('preserves version number', async () => {
            const created = await ideaService.createIdea(userId, 'Title', 'Description');
            const updated = await ideaService.updateIteration(created._id.toString(), 1, { title: 'Updated' });
            expect(updated.iterations[0].version).toBe(1);
        });
        ...
        test('throws NotFoundError when idea does not exist', async () => {
            const nonExistentId = '507f1f77bcf86cd799439011';
            await expect(ideaService.updateIteration(nonExistentId, 1, { title: 'Updated' })).rejects.toThrow(errorHandler_1.NotFoundError);
        });
        test('throws NotFoundError when iteration version does not exist', async () => {
            const created = await ideaService.createIdea(userId, 'Title', 'Description');
            await expect(ideaService.updateIteration(created._id.toString(), 999, { title: 'Updated' })).rejects.toThrow(errorHandler_1.NotFoundError);
        });
        test('updates specific iteration in multi-iteration idea', async () => {
            ...
            const updated = await ideaService.updateIteration(created._id.toString(), 1, { title: 'V1 Updated' });
            expect(updated.iterations[0].title).toBe('V1 Updated');
            expect(updated.iterations[1].title).toBe('V2'); // Should not change
        });
    });
});
//# sourceMappingURL=idea.service.test.js.map